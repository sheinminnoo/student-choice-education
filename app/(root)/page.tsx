...
const ContentSection: React.FC<{
  ...
  <section className={`${pyClass} border-t border-slate-200 last:border-b`}>
    <motion.div
...
const SectionTitle: React.FC<{
  ...
  >
    {subtitle && (
      <p className="text-yellow-500 font-semibold mb-2 tracking-[0.22em] uppercase text-[0.65rem] sm:text-xs">
        {subtitle}
      </p>
    )}
...
/* HERO – now on light background */

const HeroSection: React.FC = () => (
  <section className="relative overflow-hidden border-b border-slate-200 bg-slate-50">
    {/* soft glows kept but work on light bg */}
    ...
    >
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)] items-center">
        ...
        <div className="will-change-transform">
          <motion.div
            variants={fadeInUpFast}
            className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/90 px-3 py-1 mb-4 text-[0.7rem] sm:text-xs text-slate-700"
          >
          ...
          <motion.h1
            variants={fadeInUp}
            className="text-[2.1rem] sm:text-4xl md:text-5xl lg:text-[3.3rem] leading-tight font-extrabold tracking-tight text-slate-900 mb-4"
          >
            Find the <span className="text-yellow-500">right university</span>{" "}
            for your future.
          </motion.h1>

          <motion.p
            variants={fadeInUp}
            className="text-sm sm:text-lg text-slate-600 max-w-xl mb-7"
          >
          ...
          >
            <button
              className="px-6 py-2.5 sm:px-7 sm:py-3 text-sm sm:text-base font-semibold rounded-xl shadow-md md:shadow-lg transition duration-300 hover:translate-y-[-2px] hover:shadow-yellow-400/40 active:translate-y-[0px]"
              style={{
            ...
            <button className="px-6 py-2.5 sm:px-7 sm:py-3 text-sm sm:text-base font-semibold rounded-xl border border-slate-300 bg-white text-slate-900 hover:bg-slate-50 hover:translate-y-[-2px] active:translate-y-[0px] transition duration-300">
        ...
        <motion.div variants={fadeInUp} className="hidden lg:block">
          <div className="rounded-3xl border border-slate-200 bg-slate-900 text-slate-50 shadow-[0_20px_60px_rgba(15,23,42,0.6)] p-6 flex flex-col gap-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.25em] text-yellow-200/90">
                  Snapshot
                </p>
              ...
              <div className="rounded-full border border-yellow-300/70 px-3 py-1 text-[0.7rem] text-yellow-200 bg-yellow-500/15">
                Live updated
              </div>
            </div>
            ...
            <div className="mt-2 rounded-2xl bg-slate-800 border border-slate-700 px-4 py-3 text-xs text-slate-100">
  ...
  return (
    <ContentSection>
      ...
      >
            ...
            {valueProps.map((prop, index) => (
              ...
              >
                <CheckCircle
                  ...
                  style={{ color: EMERALD }}
                />
                <p className="ml-3 sm:ml-4 text-sm sm:text-lg text-slate-700">
                  {prop}
                </p>
              </motion.div>
            ))}
        ...
        <motion.div
          variants={fadeInUp}
          className="hidden lg:block h-80 lg:h-96 rounded-3xl border border-[rgba(250,204,21,0.3)] bg-slate-950/80 shadow-[0_25px_70px_rgba(0,0,0,0.8)] relative overflow-hidden"
        >
          ...
          <div className="relative z-10 flex h-full flex-col items-center justify-center px-10 text-center text-slate-100 text-sm">
            <p className="uppercase tracking-[0.25em] text-[0.65rem] text-yellow-200/90 mb-3">
        ...
        {destinations.map((dest, index) => (
          <motion.div
            ...
            className="
              ...
              bg-white
              shadow-sm md:shadow-[0_18px_55px_rgba(15,23,42,0.18)]
              transition-transform duration-300 hover:-translate-y-1.5
            "
          >
            <div className="h-32 sm:h-40 bg-slate-100 flex items-center justify-center text-slate-500 text-[0.7rem] sm:text-xs">
              [Image: {dest.image}]
            </div>
            <div className="flex flex-col flex-grow p-5 sm:p-6">
              ...
              <a
                href={dest.link}
                className="inline-flex items-center text-xs sm:text-sm font-semibold text-yellow-600 hover:text-yellow-500 mt-auto"
              >
                Explore destination
                <ChevronRight
                  size={16}
...
const HomePage: React.FC = () => {
  return (
    <div className="font-sans min-h-full bg-white text-slate-900">
      <HeroSection />
      <StatsSection />
      <CoreFeaturesSection />
